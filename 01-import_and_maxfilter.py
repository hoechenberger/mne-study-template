...
def find_bad_channels(raw, subject, session, task, run):
    ...
    #     (raw.copy()
    #      .pick(bad_mag_picks)
    #      .plot_sensors(ch_type='mag', axes=ax['topo_mag'], title=''))
    # else:
    #     raw.plot_sensors(axes=ax['topo_mag'])
    #     del ax['topo_mag'].collections[0]

    # if bad_grad_picks:
    #     (raw.copy()
    #      .pick(bad_grad_picks)
    #      .plot_sensors(ch_type='grad', axes=ax['topo_grad'], title=''))
    # else:
